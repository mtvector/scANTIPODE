...
#SBATCH --array=26-26# Create an array job for each line in notebooks.txt
...
NOTEBOOK=$(sed -n "${SLURM_ARRAY_TASK_ID}p" ~/Matthew/code/scANTIPODE/examples/improve_fit/notebooks.txt)
...
OUTPUT_HTML=~/Matthew/code/scANTIPODE/examples/improve_fit/outputs/"executed_$(basename "${NOTEBOOK}")" 
