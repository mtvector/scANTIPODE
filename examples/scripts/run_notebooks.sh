...
#SBATCH --array=65-65# Create an array job for each line in notebooks.txt
...
NOTEBOOK=$(sed -n "${SLURM_ARRAY_TASK_ID}p" ~/Matthew/code/scANTIPODE/examples/scripts/notebooks.txt)
...
OUTPUT_HTML=~/Matthew/code/scANTIPODE/examples/outputs/"executed_$(basename "${NOTEBOOK}")" 
