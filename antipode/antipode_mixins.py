...
class AntipodeTrainingMixin:
    ...
    def common_training_loop(self, dataloader, max_steps, scheduler, svi, print_every, device, steps=0):
        self.losses = []
        pbar = tqdm.tqdm(total=max_steps, position=0,mininterval=300.)
    ...
    def calculate_cluster_params(self, flavor:['torch','numpy'] = 'numpy',prefix='',cluster_count_threshold=50):
        ...
        if flavor == 'torch':
            ...
            constitutive_intercept = pstore.get(
            ...
            cluster_params=((prop_locs@pstore['z_decoder_weight'])+prop_cluster_intercept+torch.mean(discov_da,0,keepdims=True)+constitutive_intercept)
            ...
            discov_cluster_params=(torch.einsum('dpm,dmg->dpg',prop_locs+prop_discov_dm,pstore['z_decoder_weight']+pstore['discov_dc'])+(prop_cluster_intercept+prop_discov_di+discov_da.unsqueeze(1)+constitutive_intercept.unsqueeze(0).unsqueeze(1)))-pstore['softmax_shift']
            return (
            ...
            cluster_params=((prop_locs@pstore['z_decoder_weight'])+prop_cluster_intercept+np.mean(discov_da,0,keepdims=True)+constitutive_intercept)
            cluster_params=cluster_params[cluster_index,:]
            ...
            discov_cluster_params=(np.einsum('dpm,dmg->dpg',prop_locs+prop_discov_dm,pstore['z_decoder_weight']+pstore['discov_dc'])+(prop_cluster_intercept+prop_discov_di+np.expand_dims(discov_da,1)+constitutive_intercept[np.newaxis,np.newaxis,:]))-pstore['softmax_shift']
            if self.discov_loc == "obsm":
                discov = self._get_discov_matrix()
    ...
    def get_posterior_cluster_means(self, batch_size=128, device='cpu'):
        ...
        with torch.no_grad():
            for batch in tqdm.tqdm(loader,mininterval=300.):
                # move to device
    ...
    def sub_model_log_residuals(
        ...
        discov_cluster_params = (
            discov_cluster_expr + di_plus_ci + dcd_unsqueezed + constitutive_intercept.unsqueeze(0).unsqueeze(1)
    ...
    def _initialize_model(cls, adata, attr_dict,param_store_path,device):
        ...
        if 'discov_constitutive_de' in pstore and 'discov_da' not in pstore:
            ...
            try:
                ...
                pass
        if 'constitutive_intercept' not in pstore:
            pstore['constitutive_intercept'] = torch.zeros(
                model.num_var,
                device=device,
            )
        return model
